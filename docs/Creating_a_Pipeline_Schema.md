...
When the optional parameters is set to `true` for the sql_clause, the pipeline will check that the field in `field` exists before adding the `sql` to the query. 
...
CSVs and Queries are both operated as Pandas Dataframes once loaded to the pipeline, but are loaded differently.
